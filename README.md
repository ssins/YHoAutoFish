...
Gitee 发行版的实际附件需要额外通过 `attach_files` 接口查询，不能只依赖 `releases/latest` 返回的 `assets`。如果 Gitee 因单文件体积限制只能上传分卷，建议附件命名为 `YHoAutoFish-vX.Y.Z-windows.zip.001`、`.002` 这类连续后缀；程序会自动下载并合并为完整 zip。手工维护 `latest.json` 时，也可以显式提供 `gitee_asset_parts` / `gitee_parts` 来声明分卷列表。发布脚本现在会在检测到本地分卷时自动把 `gitee_asset_parts`、`github_html_url`、`gitee_html_url`、`github_sha256` 和 `gitee_sha256` 写进 `release/latest.json`，减少手工维护出错。若 GitHub 单包与 Gitee 分卷不是同一个二进制 zip，必须同时提供 `github_sha256` 和 `gitee_sha256`，否则 Gitee 合并包会因哈希不同而拒绝安装。
...
当前源码入口使用 `#! python3.9`，在 Windows `py main.py` 会优先调用已安装依赖的 Python 3.9。源码运行同样会请求管理员权限。
